const items = [
    {
        icon: (
            <svg className="h-5 w-5 shrink-0 text-white/90" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    return (
        <div className="border-b border-brand-cta-dark/30 bg-brand-cta text-white">
            <div className="store-shell">
                <div className="topbar-marquee h-10">
                    <div className="topbar-marquee-track">
                        {[...items, ...items, ...items, ...items, ...items, ...items].map((item, index) => (
                            <span
                                key={index}
                                className="inline-flex items-center gap-2 text-[12px] font-medium uppercase tracking-[0.14em] text-white/88 sm:text-[13px]"
                            >
                                {item.icon}
                                {item.text}
                            </span>
