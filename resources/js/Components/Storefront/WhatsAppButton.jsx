...
export default function WhatsAppButton() {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2">
            {showTooltip && (
                <div className="relative flex items-center gap-2 bg-white text-brand-text text-sm rounded-2xl shadow-lg px-4 py-3 max-w-[220px] animate-fade-in">
                    <span>¿Tenés alguna duda o necesitás ayuda? ¡Escribinos!</span>
                    <button
                        onClick={() => { setShowTooltip(false); setDismissed(true); }}
                        className="absolute -top-2 -right-2 bg-gray-200 hover:bg-gray-300 text-gray-600 rounded-full w-5 h-5 flex items-center justify-center text-xs leading-none"
                        aria-label="Cerrar"
                    >
                        ×
                    </button>
                    {/* Arrow pointing down-right toward the button */}
                    <span className="absolute bottom-[-8px] right-6 w-0 h-0 border-l-8 border-r-0 border-t-8 border-l-transparent border-t-white" />
                </div>
            )}
