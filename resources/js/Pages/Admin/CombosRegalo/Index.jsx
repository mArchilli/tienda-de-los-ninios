...
const Icons = {
    ...
    card:    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17h6M11 19h2M4 6h11v8H4zM15 9h2.5l2.5 2.5V14h-5zM7 17a1.5 1.5 0 100 3 1.5 1.5 0 000-3zm10 0a1.5 1.5 0 100 3 1.5 1.5 0 000-3z" />,
};
...
// ─── Configuración del mensaje de regalo ──────────────────────────────────────

function GiftMessageSettingsForm({ initialMaxLength }) {
    const [savedValue, setSavedValue] = useState(initialMaxLength);
    const [value, setValue]           = useState(String(initialMaxLength));
    const [saving, setSaving]         = useState(false);
    const [error, setError]           = useState(null);

    const parsed = parseInt(value, 10);
    const valid = Number.isFinite(parsed) && parsed >= 10 && parsed <= 1000;
    const dirty = valid && parsed !== savedValue;

    const handleSave = () => {
        if (!valid) {
            setError('Ingresá un número entre 10 y 1000.');
            return;
        }
        setSaving(true);
        setError(null);
        router.post(
            route('admin.combos-regalo.gift-message-settings'),
            { max_length: parsed },
            {
                preserveScroll: true,
                preserveState: true,
    ...
    return (
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <label htmlFor="gift-message-max-length" className="text-xs font-semibold uppercase tracking-wide text-brand-text-muted">
                Máximo de caracteres para el mensaje de la tarjeta de regalo
            </label>
            <p className="mt-1 text-xs text-brand-text-muted">
                Define cuántos caracteres puede escribir el cliente en el mensaje personalizado al comprar un combo de regalo.
            </p>
            <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center">
                <input
                    ...
                    className="w-32 rounded-lg border border-gray-200 px-3 py-2 text-sm text-brand-text outline-none transition focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20"
                />
                <button
                    type="button"
                    onClick={handleSave}
...
function ComboFormModal({ open, onClose, sizes, genders = [], combo = null }) {
    ...
    useEffect(() => {
        ...
        axios.get(route('admin.combos-regalo.categories-for-sizes'), {
            params,
    ...
    const submit = (e) => {
        ...
        const routeName = isEdit ? route('admin.combos-regalo.update', combo.id) : route('admin.combos-regalo.store');
    ...
    return (
        <Modal open={open} onClose={handleClose} title={isEdit ? 'Editar Combo de Regalo' : 'Crear Combo de Regalo'} size="4xl">
                ...
                <div className="rounded-xl border border-gray-200 bg-brand-bg/40 p-4 space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Ej: Combo Regalo Bebé"
...
function DeleteModal({ open, onClose, combo }) {
    ...
    const submit = () => {
        setProcessing(true);
        router.delete(route('admin.combos-regalo.destroy', combo.id), {
            onSuccess: onClose,
            onFinish: () => setProcessing(false),
        });
    };
    ...
    return (
        <Modal open={open} onClose={onClose} title="Eliminar Combo de Regalo">
        ...
        >
            ...
            <div className="relative aspect-[3/4] bg-gray-50">
                ...
                <div className="absolute top-2 left-2 flex flex-col items-start gap-1">
                    ...
                    <span className="inline-flex items-center gap-1 rounded-full bg-brand-text px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white shadow">
...
function BulkDeleteModal({ open, onClose, selectedIds, onSuccess }) {
    ...
    const submit = () => {
        setProcessing(true);
        router.delete(route('admin.combos-regalo.bulk-destroy'), {
...
export default function Index({ combos, sizes, categories, genders = [], filters, needsReviewCount = 0, giftMessageMaxLength = 150 }) {
    ...
    const applyFilters = useCallback((newSearch, newCategory, newOnlyReview) => {
        ...
        router.get(route('admin.combos-regalo.index'), params, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    const resetFilters = () => {
        ...
        setActiveCategory('');
        setOnlyReview(false);
        router.get(route('admin.combos-regalo.index'), {}, { replace: true });
    };
    ...
    return (
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <h1 className="text-xl font-bold text-brand-text">
                            {selectionMode ? 'Selección' : 'Combos de Regalo'}
                        </h1>
        ...
        >
            <Head title="Combos de Regalo" />

            <div className="p-6 space-y-5">
                <FlashBanner message={flashMsg} onDismiss={() => setFlashMsg(null)} />

                <GiftMessageSettingsForm initialMaxLength={giftMessageMaxLength} />
                ...
                {(combos.data?.length ?? 0) === 0 && (
                    <div className="flex flex-col items-center justify-center py-24 text-brand-text-muted">
                        ...
                        {hasFilters ? (
                            ...
                            <>
                                <p className="text-lg font-semibold text-brand-text">No hay combos de regalo registrados</p>
                                <p className="text-sm mt-1">Creá el primer combo de regalo para empezar</p>
                                <button
                                    onClick={() => setCreateOpen(true)}
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
                            ...
                            <div className="flex-1">
                                <h2 className="text-base font-bold text-brand-text">Todos los combos de regalo</h2>
                                <p className="text-xs text-brand-text-muted">
