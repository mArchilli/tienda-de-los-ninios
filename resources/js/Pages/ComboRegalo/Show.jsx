...
// ─── Combo de Regalo / Builder ────────────────────────────────────────────────
// Misma lógica que el combo tradicional (talle + prendas por categoría), pero
// pensado para regalo: se entrega en bolsa especial de la tienda junto con una
// tarjeta que puede llevar un mensaje personalizado que el cliente escribe acá.
...
export default function ComboRegaloShow({ combo, giftMessageMaxLength = 150, cartCount = 0 }) {
    ...
    const giftMessageTooLong = giftMessage.length > giftMessageMaxLength;
    ...
    const pickSize = (sid) => {
        if (sid === size) return;
        if (!feasibleSizes.has(sid)) return;
        setSize(sid);
        setPicks({});
    };
    ...
    const handleAdd = () => {
        if (!allComplete || giftMessageTooLong) return;
        router.post('/carrito/combo-regalo', {
            ...
            onError: (errs) => {
                setFeedback(errs?.picks || errs?.gift_message || errs?.stock || 'No pudimos agregar el combo. Intentá de nuevo.');
            },
        });
    };
        ...
        <StorefrontLayout cartCount={cartCount}>
            ...
            <div className="mx-auto max-w-screen-2xl px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
                    ...
                    <div className="relative overflow-hidden rounded-[1.9rem] bg-brand-secondary-light border border-brand-cta/30 shadow-[0_18px_40px_rgba(41,50,65,0.08)]">
                        ...
                        <span className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-full bg-brand-text px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.18em] text-white shadow-md">
                            <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M20 12v9H4v-9M2 7h20v5H2V7zm10 0v14M12 7c0-1.657-1.79-5-4-5S4 5.343 4 7m8 0c0-1.657 1.79-5 4-5s4 3.343 4 5" />
                            </svg>
                            Combo de Regalo
                        </span>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="rounded-[1.9rem] border border-brand-cta/25 bg-white p-6 shadow-[0_18px_36px_rgba(41,50,65,0.06)] sm:p-7">
                            <p className="text-[11px] uppercase tracking-[0.24em] text-brand-cta font-bold">
                                Combo de Regalo · Armalo a tu medida
                            </p>
                        ...
                        <div className="flex items-center gap-3 rounded-[1.4rem] border border-brand-cta/45 bg-white px-4 py-4 shadow-[0_12px_28px_rgba(255,90,78,0.10)]">
                            ...
                            <p className="text-sm leading-relaxed text-brand-text">
                                Armá tu combo en <span className="font-bold text-brand-cta">{1 + combo.categories.length} pasos</span>: elegí talle y los productos de cada categoría. Al final podés sumar un mensaje para la tarjeta.
                            </p>
                        </div>

                        <div className="flex flex-col gap-4 rounded-[1.4rem] border border-brand-cta/45 bg-brand-primary-surface/35 p-4 shadow-[0_12px_28px_rgba(255,90,78,0.08)] sm:flex-row sm:items-center">
                            <button
                                type="button"
                                onClick={() => {
                                    setPreviewSlides([{ src: '/images/combo-regalo-presentacion.jpg', alt: 'Bolsa y tarjeta de regalo de La Tienda de los Niños' }]);
                                    setPreviewIndex(0);
                                }}
                                className="group relative w-full shrink-0 overflow-hidden rounded-[1.1rem] border border-brand-cta/25 shadow-sm sm:w-36"
                                aria-label="Ver más grande la bolsa y tarjeta de regalo"
                            >
                                <img
                                    src="/images/combo-regalo-presentacion.jpg"
                                ...
                                />
                                <span className="absolute inset-0 flex items-end justify-center bg-black/0 pb-2 opacity-0 transition-all duration-200 group-hover:bg-black/20 group-hover:opacity-100">
                                    <span className="rounded bg-black/70 px-2 py-0.5 text-[9px] font-semibold text-white">Ampliar</span>
                                </span>
                            </button>
                            <div className="flex items-start gap-3">
                                ...
                                <p className="text-sm leading-relaxed text-brand-text">
                                    <span className="font-extrabold text-brand-text">PRESENTACIÓN DE REGALO:</span>{' '}
                                    Se entrega en bolsa especial de La Tienda de los Niños, con una tarjeta que puede incluir tu mensaje personalizado.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <section className="mt-10 space-y-4">
                    ...
                    <div className="rounded-[1.6rem] border border-brand-cta/45 bg-white p-6 shadow-[0_18px_34px_rgba(255,90,78,0.10)]">
                        <div className="flex items-center gap-3">
                            <span className="flex h-7 w-7 items-center justify-center rounded-full border border-brand-cta/35 bg-brand-cta/10 text-brand-cta">
                                <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 17h6M11 19h2M4 6h11v8H4zM15 9h2.5l2.5 2.5V14h-5zM7 17a1.5 1.5 0 100 3 1.5 1.5 0 000-3zm10 0a1.5 1.5 0 100 3 1.5 1.5 0 000-3z" />
                                </svg>
                            </span>
                            <span className="text-sm font-bold uppercase tracking-[0.18em] text-brand-cta">
                                Tarjeta de regalo
                                <span className="ml-2 inline-flex items-center rounded-full border border-brand-cta/20 bg-brand-cta/10 px-2.5 py-1 text-xs text-brand-cta/85 normal-case tracking-normal font-normal">
                                    · Opcional
                                </span>
                            </span>
                        </div>
                        <p className="mt-3 text-sm leading-relaxed text-brand-text-muted">
                            Escribí el mensaje que va a llevar la tarjeta que acompaña la bolsa de regalo.
                        </p>
                        <textarea
                            value={giftMessage}
                            ...
                            placeholder="Ej: ¡Feliz cumple! Con todo nuestro cariño."
                            className={`mt-3 w-full rounded-[1rem] border bg-white px-4 py-3 text-sm text-brand-text shadow-sm outline-none transition focus:ring-2 ${
                                giftMessageTooLong
                                    ? 'border-red-400 focus:ring-red-200'
                                    : 'border-brand-cta/30 focus:border-brand-cta focus:ring-brand-cta/15'
                            }`}
                        />
                        <p className={`mt-1.5 text-right text-[11px] font-semibold ${giftMessageTooLong ? 'text-red-500' : 'text-brand-text-muted'}`}>
                            {giftMessage.length} / {giftMessageMaxLength}
                        </p>
                    </div>
                </section>
                ...
                <section className="mt-10 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8" ref={(el) => (stepRefs.current['summary'] = el)}>
                    ...
                    <aside className="self-start rounded-[1.8rem] border border-brand-secondary/30 bg-white p-6 shadow-[0_18px_36px_rgba(41,50,65,0.08)] lg:sticky lg:top-32">
                        ...
                        <dl className="mt-4 space-y-2 text-sm">
                            ...
                            {giftMessage.trim() && (
                                <div className="flex justify-between gap-3">
                                    <dt className="text-brand-text-muted">Mensaje</dt>
                                    <dd className="max-w-[65%] text-right text-xs italic text-brand-text">
                                        &quot;{giftMessage.trim()}&quot;
                                    </dd>
                                </div>
                            )}
                        ...
                        <button
                            type="button"
                            onClick={handleAdd}
                            disabled={!allComplete || giftMessageTooLong}
                        ...
                        {!allComplete && (
                            <p className="mt-3 text-[11px] text-brand-text-muted text-center">
                                Completá talle y prendas para continuar.
                            </p>
                        )}
                        {feedback && (
