...
const COURIERS = ['Correo Argentino', 'Andreani', 'Vía Cargo', 'Asesorarme por la tienda'];
            ...
            <ul className="mt-4 divide-y divide-brand-primary/12">
                {items.map((it) => (
                    <li key={it.key} className="flex items-start gap-4 py-3">
                        {it.image ? (
                            <img src={it.image} alt={it.name} className="h-14 w-14 shrink-0 border border-brand-primary/20 bg-white object-cover" />
                        ) : (
                            <div className="h-14 w-14 shrink-0 border border-brand-primary/20 bg-brand-primary-surface" />
                        )}
                        <div className="min-w-0 flex-1">
                            ...
                            {it.type === 'combo' && (
                                <div className="mt-0.5 flex flex-wrap gap-1">
                                    {it.gender_name && (
                                        <span className="inline-flex items-center border border-brand-cta/20 bg-brand-cta-surface px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-brand-cta">
                                            {it.gender_name}
                                        </span>
                                    )}
                                    <span className="inline-flex items-center border border-brand-primary/20 bg-brand-secondary-surface px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.1em] text-brand-text-muted">
                                        Combo
                                    </span>
                                </div>
                            )}
    ...
    return (
        <StorefrontLayout>
            ...
            <form onSubmit={submit} className="mx-auto max-w-screen-2xl px-4 py-10 sm:px-6 lg:px-8">
                ...
                <div className="mb-6 flex items-start gap-4 border border-[#25D366]/40 bg-[#25D366]/8 px-5 py-4">
                    ...
                    <div>
                        <p className="text-sm font-bold text-brand-text">¿Cómo funciona?</p>
                        <ol className="mt-1.5 space-y-1 text-sm text-brand-text-muted list-none">
                            <li><span className="font-semibold text-brand-text">1.</span> Tu pedido se enviará por <span className="font-semibold text-[#25D366]">WhatsApp</span> a la tienda al confirmar.</li>
                            <li><span className="font-semibold text-brand-text">2.</span> Coordinamos el envío y te informamos el costo.</li>
                            <li><span className="font-semibold text-brand-text">3.</span> Una vez cerrado el envío, se acuerda el pago.</li>
                        </ol>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
                    ...
                    <aside className="space-y-3 self-start border border-brand-primary/25 bg-white p-5 shadow-[0_18px_36px_rgba(41,50,65,0.08)]">
                        ...
                        <button
                            type="submit"
                            disabled={processing}
                            className="mt-2 flex w-full items-center justify-center gap-2 bg-[#25D366] py-3 text-center text-sm font-bold uppercase tracking-[0.12em] text-white transition-colors hover:bg-[#1ebe5d] disabled:cursor-not-allowed disabled:opacity-50"
                        >
