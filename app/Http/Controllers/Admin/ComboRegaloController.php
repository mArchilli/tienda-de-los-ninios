...
class ComboRegaloController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $reviewIds = ComboRegalo::whereNotNull('gender_id')
            ->with(['items:id,combo_regalo_id,product_id', 'items.product:id', 'items.product.genders:id'])
            ->get(['id', 'gender_id'])
            ->filter(fn (ComboRegalo $c) => $c->items->contains(fn ($it) =>
                $it->product && ! $it->product->genders->contains('id', $c->gender_id)
            ))
            ->pluck('id');

        $combos = ComboRegalo::with([
                'sizes',
                'gender',
                'items.category',
            ...
            ->paginate(12)
            ->withQueryString()
            ->through(fn (ComboRegalo $combo) => array_merge($combo->toArray(), [
                'review' => $this->genderReview($combo),
            ]));

        return Inertia::render('Admin/CombosRegalo/Index', [
            'combos'              => $combos,
    ...
    /**
     * Detecta si un combo de regalo ofrece prendas cuyo género no coincide con
     * el suyo. Devuelve el detalle para el aviso del panel, o null si está
     * consistente. Requiere que el combo venga con `gender`, `items.category`
     * e `items.product.genders` precargados.
     */
    private function genderReview(ComboRegalo $combo): ?array
    {
        if (! $combo->gender_id) {
            return null;
        }
    ...
    public function updateGiftMessageSettings(Request $request)
    {
        $request->validate([
            'max_length' => 'required|integer|min:10|max:1000',
        ]);

        Setting::set(Setting::GIFT_MESSAGE_MAX_LENGTH_KEY, (string) $request->max_length);

        return back()->with('success', 'Configuración de mensaje de regalo actualizada.');
    }
    ...
    public function store(Request $request)
    {
        ...
        return back()->with('success', 'Combo de regalo creado correctamente.');
    }

    public function update(Request $request, ComboRegalo $combo)
    {
        $request->validate([
            'name'                         => 'required|string|max:255',
            'description'                  => 'nullable|string',
        ...
        return back()->with('success', 'Combo de regalo actualizado correctamente.');
    }

    public function destroy(ComboRegalo $combo)
    {
        if ($combo->image) {
            @unlink(public_path($combo->image));
        }
        ...
        return back()->with('success', 'Combo de regalo eliminado correctamente.');
    }
    ...
    public function bulkDestroy(Request $request)
    {
        $request->validate([
            ...
            'ids.*' => 'exists:combo_regalos,id',
        ]);

        $combos = ComboRegalo::whereIn('id', $request->ids)->get();
    ...
    private function uploadImage(\Illuminate\Http\UploadedFile $file): string
    {
        ...
        $destination = public_path($base . '/combos-regalo');
        ...
        return $base . '/combos-regalo/' . $filename;
    }
}
